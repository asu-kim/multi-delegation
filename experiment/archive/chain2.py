...
LOG_FILE = f"chain2_log_{int(time.time())}.txt"
...
def main():
    ...
    logging.info("\n=== LATENCY RESULT (process start -> first delegation (Users -> Node1) response) ===")
    ...
    logging.info("\n=== LATENCY RESULT (process start -> second delegation (Node1 -> Node2) response) ===")
    logging.info(f"{delegation2_latency_sec:.6f} seconds")
    logging.info(f"{delegation2_latency_sec * 1000:.2f} ms\n\n")


    logging.info("\n=== Start process: Node2 access ResourceA ===")
    ...
    logging.info("\n=== LATENCY RESULT (process start -> access (Node2 -> ResourceA) response) ===")
    ...
    logging.info("Total sum of all stagse: delegation1, delegation2, access")
    latency_sum =  access_latency_sec + delegation2_latency_sec + delegation1_latency_sec
