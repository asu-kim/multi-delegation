...
LOG_FILE = f"scenario1_delegate_log_{int(time.time())}.txt"
...
def main():
    ...
    logging.info("\n=== Start process: Node0 delegateAuthority Node1 ResourceA 1*day ===")
    node_cmd = "node user.js configs/net1/node0.config"
    send_command = "delegateAuthority Node1 ResourceA 1*day"
    delegation0A_latency_sec = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
    ...
    logging.info("\n=== LATENCY RESULT (process start -> ResourceA (Node0 -> Node1) response) ===")
    logging.info(f"{delegation0A_latency_sec:.6f} seconds")
    logging.info(f"{delegation0A_latency_sec * 1000:.2f} ms\n\n")

    logging.info("\n=== Start process: Node0 delegateAuthority Node1 ResourceB 1*day ===")
    node_cmd = "node user.js configs/net1/node0.config"
    send_command = "delegateAuthority Node1 ResourceB 1*day"
    delegation0B_latency_sec = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
        timeout_sec=args.timeout,
    ...
    logging.info("\n=== LATENCY RESULT (process start -> ResourceB (Node0 -> Node1) response) ===")
    logging.info(f"{delegation0B_latency_sec:.6f} seconds")
    logging.info(f"{delegation0B_latency_sec * 1000:.2f} ms\n\n")
    ...
    logging.info("\n=== Start process: Node1 delegateAuthority Node3 ResourceA 1*day ===")
    node_cmd = "node user.js configs/net1/node1.config"
    send_command = "delegateAuthority Node3 ResourceA 1*day"
    delegation1A_latency_sec = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
        timeout_sec=args.timeout,
    ...
    logging.info("\n=== LATENCY RESULT (process start -> ResourceA (Node1 -> Node3) response) ===")
    logging.info(f"{delegation1A_latency_sec:.6f} seconds")
    logging.info(f"{delegation1A_latency_sec * 1000:.2f} ms\n\n")

    logging.info("\n=== Start process: Node1 delegateAuthority Node4 ResourceB 1*day ===")
    node_cmd = "node user.js configs/net1/node1.config"
    send_command = "delegateAuthority Node4 ResourceB 1*day"
    delegation1B_latency_sec = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
        timeout_sec=args.timeout,
    ...
    logging.info("\n=== LATENCY RESULT (process start -> ResourceB (Node1 -> Node4) response) ===")
    logging.info(f"{delegation1B_latency_sec:.6f} seconds")
    logging.info(f"{delegation1B_latency_sec * 1000:.2f} ms\n\n")

    logging.info("\n=== Start process: Node2 delegateAuthority Node5 ResourceC 1*day ===")
    node_cmd = "node user.js configs/net1/node2.config"
    send_command = "delegateAuthority Node5 ResourceC 1*day"
    delegation2C_latency_sec = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
        timeout_sec=args.timeout,
    ...
    logging.info("\n=== LATENCY RESULT (process start -> ResourceC (Node2 -> Node5) response) ===")
    logging.info(f"{delegation2C_latency_sec:.6f} seconds")
    logging.info(f"{delegation2C_latency_sec * 1000:.2f} ms\n\n")

    logging.info("\n=== Start process: Node2 delegateAuthority Node6 ResourceD 1*day ===")
    node_cmd = "node user.js configs/net1/node2.config"
    send_command = "delegateAuthority Node6 ResourceD 1*day"
    delegation2D_latency_sec = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
        timeout_sec=args.timeout,
    ...
    logging.info("\n=== LATENCY RESULT (process start -> ResourceD (Node2 -> Node6) response) ===")
    logging.info(f"{delegation2D_latency_sec:.6f} seconds")
    logging.info(f"{delegation2D_latency_sec * 1000:.2f} ms\n\n")


    results, success_rate, access_latency = check_all_authorized_accesses(
        workdir=workdir,
        timeout_sec=args.timeout,
    )
    ...
    logging.info("Total sum of all stages: ")
    latency_sum =  access_latency + delegation0A_latency_sec + delegation0B_latency_sec + delegation0C_latency_sec + delegation0D_latency_sec + delegation1A_latency_sec + delegation1B_latency_sec + delegation2C_latency_sec + delegation2D_latency_sec
    logging.info(f"{latency_sum:.6f} seconds")
    logging.info(f"{latency_sum * 1000:.2f} ms\n")
    logging.info("Total sum of all delegation: ")
    latency_sum = delegation0A_latency_sec + delegation0B_latency_sec + delegation0C_latency_sec + delegation0D_latency_sec + delegation1A_latency_sec + delegation1B_latency_sec + delegation2C_latency_sec + delegation2D_latency_sec
    logging.info(f"{latency_sum:.6f} seconds")
    logging.info(f"{latency_sum * 1000:.2f} ms\n")
    logging.info("Total sum of all access: ")
