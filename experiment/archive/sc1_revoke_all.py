...
ERROR_RESPONSE_PATTERNS = [
    re.compile(r"received an Auth alert!", re.IGNORECASE),
]
...
LOG_FILE = f"scenario1_revoke_log_{int(time.time())}.txt"
...
def run_and_measure(workdir, node_cmd, send_command, timeout_sec, pattern):
    ...
    try:
        ...
        for line in proc.stdout:
            ...
            if matches_any(line, ERROR_RESPONSE_PATTERNS):
                latency = now - t0
                return False, latency, "received an Auth alert!"
...
def check_all_authorized_accesses(workdir, timeout_sec):
    ...
    for node_name, resources in NODE_RESOURCE_MAP.items():
        node_config_num = node_name.replace("Node", "")
        ...
        for resource_name in resources:
            ...
            success, access_latency_sec, reason = run_and_measure(
                workdir=workdir,
            ...
            if success:
                ...
                logging.info("Latency until failure: %.6f seconds (%.2f ms)",
                             access_latency_sec, access_latency_sec * 1000)
    ...
    for r in results:
        if r["success"]:
            ...
            logging.info("  %s -> %s : FAIL (%.6f sec, %.2f ms) | %s",
                         r["node"], r["resource"],
                         r["latency_sec"], r["latency_sec"] * 1000,
                         r["reason"])
...
def main():
    ...
    logging.info("\n=== Start process: Node0 delegateRevoke Node1 ResourceA 1*day ===")
    node_cmd = "node user.js configs/net1/node0.config"
    send_command = "revoke Node1 ResourceA"
    result, revoke0A_latency_sec, _ = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
    ...
    logging.info(f"\n=== LATENCY RESULT (process start -> ResourceA (Node0 -> Node1) response) {result}===")
    logging.info(f"{revoke0A_latency_sec:.6f} seconds")
    logging.info(f"{revoke0A_latency_sec * 1000:.2f} ms\n\n")

    logging.info("\n=== Start process: Node0 delegateRevoke Node1 ResourceB 1*day ===")
    node_cmd = "node user.js configs/net1/node0.config"
    send_command = "revoke Node1 ResourceB"
    result, revoke0B_latency_sec, _ = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
        timeout_sec=args.timeout,
    ...
    logging.info("\n=== LATENCY RESULT (process start -> ResourceB (Node0 -> Node1) response) {result}===")
    logging.info(f"{revoke0B_latency_sec:.6f} seconds")
    logging.info(f"{revoke0B_latency_sec * 1000:.2f} ms\n\n")

    logging.info("\n=== Start process: Node0 delegateRevoke Node2 ResourceC 1*day ===")
    node_cmd = "node user.js configs/net1/node0.config"
    send_command = "revoke Node2 ResourceC"
    result, revoke0C_latency_sec, _ = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
        timeout_sec=args.timeout,
    ...
    logging.info("\n=== LATENCY RESULT (process start -> ResourceC (Node0 -> Node2) response) {result}===")
    logging.info(f"{revoke0C_latency_sec:.6f} seconds")
    logging.info(f"{revoke0C_latency_sec * 1000:.2f} ms\n\n")

    logging.info("\n=== Start process: Node0 delegateRevoke Node2 ResourceD 1*day ===")
    node_cmd = "node user.js configs/net1/node0.config"
    send_command = "revoke Node2 ResourceD"
    result, revoke0D_latency_sec, _ = run_and_measure(
        workdir=workdir,
        node_cmd=node_cmd.split(),
        send_command=send_command,
        timeout_sec=args.timeout,
    ...
    logging.info("\n=== LATENCY RESULT (process start -> ResourceD (Node0 -> Node2) response) {result}===")
    logging.info(f"{revoke0D_latency_sec:.6f} seconds")
    logging.info(f"{revoke0D_latency_sec * 1000:.2f} ms\n\n")
    ...
    logging.info("From first revokation start to access checking")
    ...
    latency_sum =  access_latency + revoke0A_latency_sec + revoke0B_latency_sec + revoke0C_latency_sec + revoke0D_latency_sec
    logging.info(f"{latency_sum:.6f} seconds")
    logging.info(f"{latency_sum * 1000:.2f} ms\n")
    logging.info("Total sum of all revokation: ")
    latency_sum =  revoke0A_latency_sec + revoke0B_latency_sec + revoke0C_latency_sec + revoke0D_latency_sec
    logging.info(f"{latency_sum:.6f} seconds")
    logging.info(f"{latency_sum * 1000:.2f} ms\n")
